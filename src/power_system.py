import numpy as np
from numpy import abs, angle, conj, cos, imag, max, pi, real, sin, sum, zeros
...
class PowerSystem:
    ...
    def remove_row_cols(
        self, matrix: np.ndarray, rm_rows: list, rm_cols: list
    ) -> np.ndarray:
        matrix = np.delete(matrix, rm_rows, axis=0)
        matrix = np.delete(matrix, rm_cols, axis=1)
        return matrix
    ...
    def newton_raphson(self):
        ...
        while self.maxerror >= self.accuracy and self.iter <= self.maxiter:
            ...
            J2 = self.remove_row_cols(J2_raw, tbr_r, tbr_c)
            ...
            for i in range(nbus_int):
                ...
                row[i] = -np.sum(row)
                rows.append(row)
            J3_raw = np.vstack(rows)

            tbr_r = bus_type_inds["Slack"] + bus_type_inds["PV"]
            tbr_c = bus_type_inds["Slack"]

            J3 = self.remove_row_cols(J3_raw, tbr_r, tbr_c)
    ...
    def lfnewton(self):
        ...
        for k in range(len(self.busdata)):
            ...
            #     self.V[n_idx] = self.Vm[n_idx] * (
            #         cos(self.delta[n_idx]) + 1j * sin(self.delta[n_idx])
            #     )
            #     self.P[n_idx] = (self.Pg[n_idx] - self.Pd[n_idx]) / self.basemva
            #     self.Q[n_idx] = (
            #         self.Qg[n_idx] - self.Qd[n_idx] + self.Qsh[n_idx]
            #     ) / self.basemva
        ...
        while self.maxerror >= self.accuracy and self.iter <= self.maxiter:
                ...
                for i in range(self.nbr):
                    if self.nl[i] - 1 == n or self.nr[i] - 1 == n:
                        ...
                        J11 += (
                            ...
                            * self.Vm[l]
                            * Ym[n, l]
                            * sin(t[n, l] - self.delta[n] + self.delta[l])
                        )
                        J33 += (
                            self.Vm[n]
                            * self.Vm[l]
                            * Ym[n, l]
                            * cos(t[n, l] - self.delta[n] + self.delta[l])
                        )
            ...
            if self.iter == self.maxiter and self.maxerror > self.accuracy:
                print(
                    f"WARNING: Iterative solution did not converge after {self.iter} iterations."
                )
                converge = 0
    ...
    def lfgauss(self):
        ...
        # Process bus data
        for k in range(len(self.busdata)):
            n = int(self.busdata[k, 0])  # Bus numbers (if not ordered already)
            n_idx = n - 1  # 0-indexed array

            self.kb[n_idx] = int(self.busdata[k, 1])
            # Bus type 1: slack / 2: PV / 0: PQ
            self.Vm[n_idx] = self.busdata[k, 2]  # Voltage magnitude
            self.deltad[n_idx] = self.busdata[k, 3]  # Voltage phase in degrees
            self.Pd[n_idx] = self.busdata[k, 4]  # Demanded active power at each bus
            self.Qd[n_idx] = self.busdata[k, 5]  # Demanded reactive power at each bus
            self.Pg[n_idx] = self.busdata[k, 6]  # Generated active power at each bus
            self.Qg[n_idx] = self.busdata[k, 7]  # Generated reactive power at each bus
            self.Qmin[n_idx] = self.busdata[k, 8]
            # Minimum generated reactive power (at pv bus)
            self.Qmax[n_idx] = self.busdata[k, 9]
            # Maximum generated reactive power (at pv bus)
            self.Qsh[n_idx] = self.busdata[k, 10]
            # Generated reactive power by shunt capacitors

            if self.Vm[n_idx] <= 0:  # magnitude can not be negative.
                # Initial voltage magnitude should(!) be between 0 and 1
                ## NOOO! Infact voltage magnitude could be fixed by PV buses to a predetermined value
                self.Vm[n_idx] = 1.0
                self.deltad[n_idx] = 0
            # else:
            #     # Convert degree to radian
            #     self.delta[n_idx] = np.pi / 180 * self.delta[n_idx]
            #     # Initial voltages in complex format
            #     self.V[n_idx] = self.Vm[n_idx] * (
            #         np.cos(self.delta[n_idx]) + 1j * np.sin(self.delta[n_idx])
            #     )
            # Calculate total active power at each bus (generated - consumed) in p.u.
            # self.P[n_idx] = (self.Pg[n_idx] - self.Pd[n_idx]) / self.basemva
            # # Calculate total reactive power at each bus (generated - consumed) in p.u.
            # self.Q[n_idx] = (
            #     self.Qg[n_idx] - self.Qd[n_idx] + self.Qsh[n_idx]
            # ) / self.basemva
            # # Calculate complex power generation at each bus
            # self.S[n_idx] = self.P[n_idx] + 1j * self.Q[n_idx]

        self.delta = np.pi / 180 * self.deltad
        self.V = np.multiply(self.Vm, (np.cos(self.delta) + 1j * np.sin(self.delta)))
        self.P = (self.Pg - self.Pd) / self.basemva
        self.Q = (self.Qg - self.Qd + self.Qsh) / self.basemva
        self.S = self.P + 1j * self.Q
        ...
        DV = np.zeros(nbus_int, dtype=float)

        # Calculate intial value of complex power with initial voltages
        Sc = np.multiply(self.V, np.conj(np.dot(self.V, self.Ybus)))
        ...
        while self.maxerror >= accuracy and self.iter <= maxiter:
            ...
            for n in range(nbus_int):
                ...
                # Create eleminated Y_bus matrix and multiply the n th row by eliminated volatage vector
                ...
                # Some buses my be disconnected (Y_bus at i,j  = 0)
                ...
                # Calculate complex power (one-liner):
                ...
                if self.kb[n] == 2:
                    ...
                    # Weird things down here 0_0
                    # if (
                    #     abs(DQ[n]) <= 0.005 and self.iter >= 10
                    # ):  # After 10 iterations
                ...
                if self.kb[n] != 1:
                    Vc[n] = (np.conj(self.S[n]) / np.conj(self.V[n]) - YV) / self.Ybus[
                        n, n
                    ]
                ...
                for i in range(self.nbr):
                    if self.nl[i] - 1 == n or self.nr[i] - 1 == n:
                        ...
                        J11 += (
                            ...
                            * self.Vm[l]
                            * Ym[n, l]
                            * np.sin(t[n, l] - self.delta[n] + self.delta[l])
                        )
                        J33 += (
                            self.Vm[n]
                            * self.Vm[l]
                            * Ym[n, l]
                            * np.cos(t[n, l] - self.delta[n] + self.delta[l])
                        )
    ...
    def perturbation(self):
        ...
        for i in range(nbus):
            ...
            self.yload[n_idx] = (Pd[n] - 1j * Qd[n] + 1j * Qsh[n]) / (
                self.basemva * Vm[n] ** 2
            )
    ...
    def busout(self):
        ...
        for n in range(int(self.nbus)):
            n_idx = n  # 0-indexed
            print(
                f"{n+1:5d} {self.Vm[n_idx]:7.3f} {self.deltad[n_idx]:8.3f} "
        ...
        for n in range(1, int(self.nbus) + 1):
            ...
            for L in range(int(self.nbr)):
                ...
                if self.nl[L] == n:  # From side
                    ...
                    In = (self.V[n_idx] - self.a[L] * self.V[k_idx]) * self.y[L] / (
                        self.a[L] ** 2
                    ) + self.Bc[L] / self.a[L] ** 2 * self.V[n_idx]
                    Ik = (self.V[k_idx] - self.V[n_idx] / self.a[L]) * self.y[
                        L
                    ] + self.Bc[L] * self.V[k_idx]
                    ...
                    print(
                        f"{k:12d} {real(Snk):9.3f} {imag(Snk):9.3f} {abs(Snk):9.3f} "
                    ...
                    # Calculate line flows
                    In = (self.V[n_idx] - self.V[k_idx] / self.a[L]) * self.y[
                        L
                    ] + self.Bc[L] * self.V[n_idx]
                    Ik = (self.V[k_idx] - self.a[L] * self.V[n_idx]) * self.y[
                        L
                    ] / self.a[L] ** 2 + self.Bc[L] / self.a[L] ** 2 * self.V[k_idx]
    ...
    def bloss(self):
        ...
        for k in range(int(self.nbus)):
            if self.kb[k] != 0:
                ...
                if self.Pg[k] > 1e-6:
                    al[kg] = (
                        1 - 1j * ((self.Qg[k] + self.Qsh[k]) / self.Pg[k])
                    ) / np.conj(self.V[k])
                else:
        ...
        T = np.matmul(
            np.matmul(np.matmul(alph, np.conj(C.T)), np.real(Zbus)),
            np.matmul(np.conj(C), np.conj(alph)),
        )
        ...
        PL = (
            np.dot(Pgg_array, np.dot(self.B / self.basemva, Pgg_array))
            + np.dot(self.B0, Pgg_array)
            + self.B00 * self.basemva
        )
    ...
    def dispatch(self, Pdt=None, cost=None, mwlimits=None):
        ...
        print(
            f"Incremental cost of delivered power (system lambda) = {lambda_:.6f} $/MWh"
        )
        ...
        if self.busdata is not None:
            ...
            for k in range(len(self.busdata)):
                ...
                if (
                    ...
                    and bus_idx < len(self.kb)
                    and self.kb[bus_idx] != 0
                ):
                    if ng < len(self.Pgg):
            ...
            if hasattr(self, "kb") and self.kb is not None:
                for k in range(int(self.nbus)):
                    if self.kb[k] == 1:  # Slack bus
                        ...
                        print(
                            f"Absolute value of the slack bus real power mismatch, dpslack = {dpslack:.4f} pu"
                        )
                        break
